...
#                 self.real = torch.flip(self.real, [3])
    ...
    def compute_D_loss(self):
        ...
        fake = self.crop(self.fake_B.detach())
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake = self.netD(fake)
        self.loss_D_fake = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netD(self.crop(self.real_B))
    ...
    def save_images(self, epoch, test_data):
        ...
        if num_img != 1:
            _, ax = plt.subplots(num_img, 3, figsize=(20, 10))
            [ax[0, i].set_title(title) for i, title in enumerate(['Source', "Translated", "Target"])]
            ...
            [ax[i].set_title(title) for i, title in enumerate(['Source', "Translated", "Target"])]
            for i, data in enumerate(test_data):
                self.set_input(data)
                self.forward()
                source = linear_normalize(self.real_A[0].permute([1,2,0]).detach().cpu().numpy())
                translated = linear_normalize(self.fake_B[0].permute([1,2,0]).detach().cpu().numpy())
                target = linear_normalize(self.real_B[0].permute([1,2,0]).detach().cpu().numpy())
                [ax[j].imshow(img) for j, img in enumerate([source, translated, target])]
                [ax[j].axis("off") for j in range(3)]
