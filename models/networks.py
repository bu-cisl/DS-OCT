...
class PatchSampleF(nn.Module):
    ...
    def create_mlp(self, feats):
        for mlp_id, feat in enumerate(feats):
            ...
            mlp = nn.Sequential(*[nn.Linear(input_nc, self.nc), nn.SiLU(), nn.Linear(self.nc, self.nc)])
...
class LPTN(nn.Module):
    def __init__(self, nrb_low=4, nrb_high=2, num_high=4):
    ...
    def forward(self, real_A_full, layers=[], encode_only=False):
        pyr_A = self.lap_pyramid.pyramid_decom(img=real_A_full)

        if -1 in layers:
            layers.append(len(self.trans_low.model))
        if len(layers) > 0:
            feat = pyr_A[-1]
            feats = []
            for layer_id, layer in enumerate(self.trans_low.model):
                feat = layer(feat)
                if layer_id in layers:
                    feats.append(feat)
                else:
