...
import cv2 as cv
import numpy as np
from util import stain_utils as ut
...
class Normalizer(object):
    ...
    def fit(self, target):
        ...
        foreground_mask = ut.notwhite_mask(target, 1.0)
        means, stds = get_mean_std(target*foreground_mask[:,:,np.newaxis])
        self.target_means = means
        self.target_stds = stds

    def transform(self, I):
        I = ut.standardize_brightness(I)
        foreground_mask = ut.notwhite_mask(I, 1.0)
        I1, I2, I3 = lab_split(I)
        means, stds = get_mean_std(I*foreground_mask[:,:,np.newaxis])
        ...
        return merge_back(norm1*foreground_mask, norm2*foreground_mask, norm3*foreground_mask) + I*(1-foreground_mask[:,:,np.newaxis])