...
def data_aug_preprocessing_512():
    data_augmented = np.ndarray(shape=(96,512,512,4))
    X = 1 - np.load('../Data/200503_Hui/x_mus.npy').astype('float32')[:,:,np.newaxis]
    Y_unpaired = np.load('../Data/200503_Hui/y_gallyas_unpaired.npy').astype('float32')
    Data = np.concatenate([X, Y_unpaired], axis=-1)
    for i in range(8):
        tmp = np.ndarray(shape=(6,512,512,4))
        x1 = randrange(0, Data.shape[0] - 512)
        y1 = randrange(0, Data.shape[1] - 512)
        tmp[0,:] = Data[x1:x1+512, y1:y1+512,:]
    ...
    X = 1 - np.load('../Data/200615_Hui/x_mus.npy').astype('float32')[:,:,np.newaxis]
    Y_unpaired = np.load('../Data/200615_Hui/y_gallyas_unpaired.npy').astype('float32')
    Data = np.concatenate([X, Y_unpaired], axis=-1)
    for i in range(8):
        tmp = np.ndarray(shape=(6,512,512,4))
        x1 = randrange(0, Data.shape[0] - 512)
        ...
        tmp[4,:] = ndimage.rotate(tmp[0,:], 180)
        tmp[5,:] = ndimage.rotate(tmp[0,:], 270)
        data_augmented[48+6*i:48+6*i+6,:,:,:] = tmp

    shuffle(data_augmented)
    x_augmented = data_augmented[:,:,:,0:1].astype('float32')
    y_augmented = data_augmented[:,:,:,1:].astype('float32')
    X_shape = x_augmented.shape[1:]
    Y_shape = y_augmented.shape[1:]
    x_augmented = np.transpose(x_augmented, [0,3,1,2])
    y_augmented = np.transpose(y_augmented, [0,3,1,2])
    return x_augmented, y_augmented

def data_aug_preprocessing_256():
    data_augmented = np.ndarray(shape=(384,256,256,4))
    X = 1 - np.load('../Data/200503_Hui/x_mus.npy').astype('float32')[:,:,np.newaxis]
    Y_unpaired = np.load('../Data/200503_Hui/y_gallyas_unpaired.npy').astype('float32')
    Data = np.concatenate([X, Y_unpaired], axis=-1)
    for i in range(32):
        tmp = np.ndarray(shape=(6,256,256,4))
        x1 = randrange(0, Data.shape[0] - 256)
        y1 = randrange(0, Data.shape[1] - 256)
        tmp[0,:] = Data[x1:x1+256, y1:y1+256,:]
        tmp[1,:] = np.fliplr(tmp[0,:])
        tmp[2,:] = np.flipud(tmp[0,:])
        tmp[3,:] = ndimage.rotate(tmp[0,:], 90)
        tmp[4,:] = ndimage.rotate(tmp[0,:], 180)
    ...
    for i in range(32):
        ...
        tmp[4,:] = ndimage.rotate(tmp[0,:], 180)
        tmp[5,:] = ndimage.rotate(tmp[0,:], 270)
        data_augmented[192+6*i:192+6*i+6,:,:,:] = tmp
...
def data_aug_preprocessing_newres():
    dim = 512
    data_augmented = np.ndarray(shape=(192,dim,dim,4))
    X = 1 - np.load('../Data/200505_Hui/x_mus.npy').astype('float32')[:,:,np.newaxis]
    Y_unpaired = np.load('../Data/200505_Hui/y_gallyas_unpaired.npy').astype('float32')
    Data = np.concatenate([X, Y_unpaired], axis=-1)
    for i in range(32):
        tmp = np.ndarray(shape=(6,dim,dim,4))
...
def data_loading_HR_full_FOV(data_folder):
    # converting to the tensor shape in tensorflow [N,H,W,C]

#     X = 1 - np.load('../Data/200505_Hui/x_mus.npy').astype('float32')[np.newaxis,:,:,np.newaxis]
#     Y = np.load('../Data/200505_Hui/y_gallyas_unpaired.npy').astype('float32')[np.newaxis,:,:,:]

#     X = 1 - np.load('../Data/200615_Hui/x_mus.npy').astype('float32')[np.newaxis,:,:,np.newaxis]
#     Y = np.load('../Data/200615_Hui/y_gallyas_unpaired.npy').astype('float32')[np.newaxis,:,:,:]

#     X = 1 - np.load('../Data/200530_Hui/x_mus.npy').astype('float32')[np.newaxis,:,:,np.newaxis]
#     Y = np.load('../Data/200530_Hui/y_gallyas_unpaired.npy').astype('float32')[np.newaxis,:,:,:]


#     X = 1 - np.load('../Data/Examples/x_mus1-1.npy').astype('float32')[np.newaxis,:,:,np.newaxis] 
#     Y = np.load('../Data/Examples/y_gallyas_SupFront.npy').astype('float32')[np.newaxis,:,:,:]
    
    X = 1 - np.load(data_folder + 'x_mus.npy').astype('float32')[np.newaxis,:,:,np.newaxis] 
    Y = np.load(data_folder + 'y_gallyas_paired.npy').astype('float32')[np.newaxis,:,:,:]
    
#     h, w = X.shape[1], X.shape[2]
#     padding_h, padding_w = int(np.ceil(h/dim)*dim-h)//2, int(np.ceil(w/dim)*dim-w)//2
#     X = np.pad(X, pad_width=((0,0),(padding_h,padding_h),(padding_w,padding_w),(0,0)), constant_values=1.0)
#     Y = np.pad(Y, pad_width=((0,0),(padding_h,padding_h),(padding_w,padding_w),(0,0)), constant_values=1.0)
    # transfering to pytorch format [N,C,H,W]
...
def LoadingImgs_to_ListOfListOfTensors(data_folder, img_index):
...
def rgb_to_lab(srgb):
    ...
    rgb_pixels = (
                (srgb_pixels / 12.92 * linear_mask) + (((srgb_pixels + 0.055) / 1.055) ** 2.4) * exponential_mask).type(
        torch.FloatTensor)
    ...
    xyz_normalized_pixels = torch.mul(xyz_pixels,
                                      torch.tensor([1 / 0.950456, 1.0, 1 / 1.088754]).type(torch.FloatTensor))
